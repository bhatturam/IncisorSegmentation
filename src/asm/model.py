...
class ActiveShapeModel:
    def __init__(self, images, shape_list, grey_model_number_of_pixels=30,
                 grey_model_search_number_of_pixels = 60,
    ...
    def fit(self,test_image,combined_fit_tol=1e-7, combined_fit_max_iters=10000):
        ...
        print current_shape.raw().shape
        for num_iter in range(combined_fit_max_iters):
            previous_shape = Shape(current_shape.raw().copy())
            new_shape_grey, error_list = self._grey_model.search(test_image,current_shape,self._grey_model_search_number_of_pixels)
            current_shape,fit_error = self._shape_model.fit(new_shape_grey,self._shape_fit_tol,self._shape_fit_max_iters)
