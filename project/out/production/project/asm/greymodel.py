...
class GreyModel:
    ...
    def _get_point_normal_slope(self, shape, point_index):
        ...
        line = cv2.fitLine(np.double(neighborhood), distType=2, param=0, reps=.01, aeps=0.01)  # 2 => CV_DIST_L2
    ...
    def _get_point_normal_pixel_coordinates_train(self, shape, point_index):
        ...
        return np.round(
            np.array([point + incr * slope for incr in range(-self._number_of_pixels, self._number_of_pixels + 1)]))
